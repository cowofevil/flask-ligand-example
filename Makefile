...
check-integration: ## verify that the Docker environment for integration testing is running (~120sec timeout)
	@rm -f /tmp/ligand_example.resp; \
	until [ -s /tmp/ligand_example.resp ] || (( count++ >= 8 )); do \
  	touch -f /tmp/ligand_example.resp; \
	curl --retry 5 --retry-delay 2 --retry-connrefused -o /tmp/ligand_example.resp -s -X GET --url \
	http://localhost:8080/realms/flask-ligand/.well-known/openid-configuration 2>&1 1>/dev/null; \
	sleep 5; \
	done; \
	if [ ! -s /tmp/ligand_example.resp ]; then echo 'Integration testing environment is not running!' && exit 1; fi
...
type-check: ## check Python types using mypy
	@mypy flask_ligand_example app.py setup.py tests
...
coverage-term: ## check code coverage with a simple terminal report
	@coverage run --source flask_ligand_example -m pytest -p no:warnings tests/unit
	@coverage report -m
...
install-editable: ## install the package in editable mode
	@if pip3 list -e | grep 'flask_ligand_example'; then echo 'Editable package already installed'; else \
	pip3 install -e .; fi
